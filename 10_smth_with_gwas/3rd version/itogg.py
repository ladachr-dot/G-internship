...
def build_enriched_table(file_path, top_n=20):
    ...
    df_sorted = df.sort_values("score", ascending=False).dropna(subset=["rsid"]).drop_duplicates(subset=["rsid"])

    return effect_used, df_sorted.head(top_n).copy()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n[!] Сортировка по {effect_used}. Начинаю сбор данных из Ensembl...")
